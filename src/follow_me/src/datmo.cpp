...
void datmo::perform_basic_clustering()
{
    ...
    for (int loop_hit = 1; loop_hit < nb_beams; loop_hit++)
    {
        ...
        geometry_msgs::Point p1 = current_scan[loop_hit - 1];
        geometry_msgs::Point p2 = current_scan[loop_hit];
        float distance = distancePoints(p1, p2);
...
void datmo::perform_advanced_clustering()
{
    ...
    for (int loop_cluster = 0; loop_cluster < nb_clusters; loop_cluster++)
    {
        ...
        geometry_msgs::Point p1 = current_scan[start];
        geometry_msgs::Point p2 = current_scan[end];
        cluster_size[loop_cluster] = distancePoints(p1, p2);
            ROS_INFO("p1 x %f y %f", p1.x, p1.y);
            ROS_INFO("p2 x %f y %f", p2.x, p2.y);

        geometry_msgs::Point pmiddle ;
        pmiddle.x = p1.x + (p2.x-p1.x)/2;
                pmiddle.y = p1.y + (p2.y-p1.y)/2;
                pmiddle.z = p1.z + (p2.z-p1.z)/2;
        cluster_middle[loop_cluster] = pmiddle;      


        int dynamic_points = compute_nb_dynamic(start, end);
        
        cluster_dynamic[loop_cluster] = (dynamic_points / (end-start+1))*100;
    }
    ...
    for (int loop=0; loop<nb_clusters; loop++)//loop over all the clusters
    {
        ...
        if((size > leg_size_min) && (size < leg_size_max)) {
            ...
            if (cluster_dynamic[loop] > dynamic_threshold) 
                leg_dynamic[nb_legs_detected] = true; //else set false
            else
...
void datmo::detect_persons()
{
    ...
    for (int loop_leg_right = 0; loop_leg_right < nb_legs_detected; loop_leg_right++)
        for (int loop_leg_left = loop_leg_right + 1; loop_leg_left < nb_legs_detected; loop_leg_left++)
        {
            ...
            geometry_msgs::Point pleg_left = leg_detected[loop_leg_left];
            geometry_msgs::Point pleg_right = leg_detected[loop_leg_right];


            float dis = distancePoints(pleg_left, pleg_right);

            if(dis < legs_distance_max && dis > legs_distance_min) {
                geometry_msgs::Point pmiddle;
                pmiddle.x = pleg_left.x + (pleg_right.x-pleg_left.x)/2;
                pmiddle.y = pleg_left.y + (pleg_right.y-pleg_left.y)/2;
                pmiddle.z = pleg_left.z + (pleg_right.z-pleg_left.z)/2;
                
                person_detected[nb_persons_detected] = pmiddle;

                leg_left[nb_persons_detected] = loop_leg_left;
                leg_right[nb_persons_detected] = loop_leg_right;

                if(leg_dynamic[loop_leg_left] || leg_dynamic[loop_leg_right]) 
                    person_dynamic[nb_persons_detected] = true;//else set false
                else
...
void datmo::detect_a_moving_person()
{
    ...
    geometry_msgs::Point r;
                ...
                r.y = 0;
                r.z = 0;
    float min_distance = distancePoints(r, person_tracked);
    for (int loop_persons = 0; loop_persons < nb_persons_detected; loop_persons++) {
        if (person_dynamic[loop_persons])
        {
            geometry_msgs::Point person = person_detected[loop_persons];

            float new_dist = distancePoints(r, person);
            // if(fabs(person.x) < fabs(person_tracked.x)) {
    ...
    if (is_person_detected) pub_datmo.publish(person_tracked);
...
void datmo::track_a_person()
{
    ...
    associated = false;
    float distance_min = uncertainty < uncertainty_max ? uncertainty : uncertainty_max;
    int index_min;
    ...
    for (int loop_persons = 0; loop_persons < nb_persons_detected; loop_persons++)
    {
        ...
        float cur_dist = distancePoints(person_detected[loop_persons], person_tracked);
        if (cur_dist < distance_min) {
            distance_min = cur_dist;
            associated = true;
    ...
    if ( associated )
    {
        ...
        // if (person_tracked.x == 0.0 && person_tracked.y == 0.0) {
        //     frequency = frequency_init;
        // } else {
    ...
    ROS_INFO("uncertainty %f", uncertainty);
    ROS_INFO("person_tracked.x %f", person_tracked.x);
    // getchar();

    // if (person_tracked != NULL) pub_datmo.publish(person_tracked);
