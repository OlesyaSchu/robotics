...
rotation_node() {
    ...
    init_odom = false;   
   cond_rotation = false;//TO COMPLETE
...
void update()
{
    ...
    if ( init_odom )
    {
        ...
        // should be if instead of do while
        if  (cond_rotation )
        {
            compute_rotation();
            move_robot();
...
void init_rotation()
{
    ...
    initial_orientation = current_orientation; //TO COMPLETE - float
    ...
    if ( translation_to_do )
    {
        ...
        // if rotation to do > rotation treshold then cond_rotation = true
