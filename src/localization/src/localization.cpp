...
/*Q1: what value of 5cms and 1 meter, manage it in m or cm
in map, 1 means 1 meter*/ 

/*Q2: how to initialize score_max
...
void localization::find_best_position(float min_x, float max_x, float min_y, float max_y, float min_orientation, float max_orientation)
{
    ...
    for(float loop_x = min_x; loop_x < max_x; loop_x+=0.05){
        for(float loop_y = min_y; loop_y < max_y; loop_y+=0.05){
            for(float o = min_orientation; o < max_orientation; o+=5*M_PI/180){
                if ( cell_value(loop_x, loop_y) == 0 ) { // robair can only be at a free cell
                    ...
                    //ROS_INFO("(%f, %f, %f): score = %i", loop_x, loop_y, o*180/M_PI, score_current);
                    if (score_max == -1) score_max = score_current;
    ...
    ROS_INFO("(estimated_position x %f, y %f, %f): score = %i", estimated_position.x, estimated_position.y, estimated_orientation, score_max);
